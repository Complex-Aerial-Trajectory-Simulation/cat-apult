...
2 checked recordings/
    2 1 checked full/       2 2 checked trunc/
    |  build_dataset.py
    v
3 dataset/  train.npz  val.npz  test.npz  manifest.json
```
...
Output lands in `2 checked recordings/`, with `2 1 checked full/` and `2 2 checked trunc/` split at identical boundaries. Each segment is re-zeroed in both position and time, so it stands alone as its own flight.

> Re-running regenerates this folder. Delete `2 checked recordings/` first so no stale segments from a previous run linger.

### 4. Verify Motion-Regime Labels

Regime labels are a heuristic, so the thresholds need a human eye before the dataset is built. This tool colors a segment's trajectory by its labels, so you can check they match what the path visibly does.

```bash
python visual_regime_check.py                     # newest checked segment
python visual_regime_check.py "2 checked recordings/2 2 checked trunc/liftoff_trunc_YOUR_SEGMENT.csv"
python visual_regime_check.py --accel-thresh 4 --turn-thresh 12
```

With no arguments it picks the newest segment in `2 checked recordings/2 2 checked trunc/`, falling back to `1 truncated recordings/`. The figure shows a 3D path, a top-down view, and a regime-vs-time strip laid over the speed trace, and it prints the label distribution to the terminal. A PNG is saved next to the CSV with a `_regimes` suffix.

|Color|Regime|
|---|---|
|Blue|`CV` - constant velocity|
...
Once the colors look right, pass the same thresholds through:
...
| `visual_regime_check.py` | 3D regime-colored path plus a top-down view and a regime-over-time graph for one recording                      |
